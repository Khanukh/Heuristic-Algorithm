...
Function_name = 'F10';
  ...
  ub=4;
  step = 1;
  dim= length(ssinks); % sink numbers
  Max_iteration=20;
%% fitness function
fobj = @F10;
...
%% algorithm
 tic
[Best_score,Best_pos,GWO_cg_curve]=GWO(SearchAgents_no,Max_iteration,lb,ub,dim);
tend = toc;
excel_ouput(1,i) = i;
excel_ouput(2,i) = tend;
excel_ouput(3,i) = sum(Best_pos>1);
[lll,llllll] = min(GWO_cg_curve);
